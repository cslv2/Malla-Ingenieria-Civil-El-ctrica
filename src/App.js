...
const esPlaceholderMencion = (c) => /\bmenci[oó]n\s+electiva\b/i.test(c.nombre.trim());
/* =================== APP =================== */
const App = () => {
    /* ----- progreso ----- */
    ...
    const [paletaClaro, setPaletaClaro] = useState("azulVerde");
    ...
    const [grupoElectivoActivo, setGrupoElectivoActivo] = useState(() => {
    ...
    const basePorSemestre = {};
    for (const curso of todosLosCursos) {
        if (!basePorSemestre[curso.semestre])
            basePorSemestre[curso.semestre] = [];
    ...
    =============================================================== */
    const cursosPorSemestre = { ...basePorSemestre };
    if (grupoElectivoActivo) {
        ...
        for (const s of Object.keys(cursosPorSemestre).map(Number)) {
            cursosPorSemestre[s] = (cursosPorSemestre[s] || []).filter((c) => !esPlaceholderMencion(c));
        }
        ...
        for (const e of delGrupo) {
            if (!cursosPorSemestre[e.semestre])
                cursosPorSemestre[e.semestre] = [];
            const ya = cursosPorSemestre[e.semestre].some((c) => c.id === e.id);
            if (!ya)
                cursosPorSemestre[e.semestre].push(e);
        }
    }
    ...
    const totalVisibles = Object.values(cursosPorSemestre).reduce((acc, arr) => acc + arr.length, 0) || 1;
    const porcentaje = (aprobadosVisibles.length / totalVisibles) * 100;
    const creditosTotales = aprobadosVisibles
        .map((id) => {
        const c = todosLosCursos.find((x) => x.id === id) ||
            electivos.find((x) => x.id === id);
        return c?.creditos || 0;
    })
        .reduce((acc, cur) => acc + cur, 0);
    /* =================== UI =================== */
    return (_jsxs("div", { className: `relative min-h-screen p-6 overflow-x-auto transition-colors duration-500 ${T.appBg}`, children: [_jsx("button", { onClick: () => setModoOscuro(!modoOscuro), className: `absolute top-4 right-6 text-sm px-3 py-1 rounded transition
        ${modoOscuro ? "bg-blue-800 text-white hover:bg-blue-700" : "bg-blue-600 text-white hover:bg-blue-500"}`, children: modoOscuro ? "🌙 Modo Oscuro" : "☀️ Modo Claro" }), !modoOscuro && (_jsxs("button", { onClick: () => setPaletaClaro((p) => (p === "azulVerde" ? "rojoNaranja" : "azulVerde")), className: "absolute top-4 left-6 text-sm px-3 py-1 rounded transition bg-slate-800 text-white hover:bg-slate-700", title: "Cambiar paleta del tema claro", children: ["\uD83C\uDFA8 Paleta: ", paletaClaro === "azulVerde" ? "Azul/Verde" : "Rojo/Naranja"] })), _jsxs("div", { className: "mt-12 mb-4 flex flex-wrap gap-2 items-center justify-center", children: [_jsx("span", { className: "text-sm opacity-70", children: "Electivos:" }), _jsx("button", { onClick: () => setGrupoElectivoActivo(null), className: [
                        ...
                        ].join(" "), children: "Ninguno" }), gruposElectivo.map((g) => {
                        const activo = grupoElectivoActivo === g;
                        return (_jsx("button", { onClick: () => setGrupoElectivoActivo(activo ? null : g), className: [
                                "px-3 py-1 rounded-md border text-sm shadow-sm transition",
                                activo
                                    ? (modoOscuro ? "bg-purple-600 text-white border-transparent"
                                        : "bg-teal-500 text-white border-transparent")
                                    : (modoOscuro ? "bg-blue-900 text-blue-100 border-blue-700 hover:bg-blue-800"
                                        : "bg-white/80 text-slate-900 border-slate-300 hover:bg-slate-100"),
                            ].join(" "), title: `Mostrar ${g} en la malla`, children: g }, g));
                    })] }), _jsx("h1", { className: "text-2xl font-bold text-center mb-4", children: "Malla Interactiva \u2013 Ingenieria Civil Electrica \u2013 U. de Antofagasta" }), _jsxs("div", { className: "text-center mb-5", children: [_jsxs("div", { className: "flex justify-center items-center gap-6 font-semibold mb-2", children: [_jsxs("p", { className: "text-lg", children: ["Avance total: ", porcentaje.toFixed(1), "%"] }), _jsxs("p", { className: "text-ms", children: ["Cr\u00E9ditos acumulados: ", _jsx("span", { className: "font-bold", children: creditosTotales })] })] }), _jsx("div", { className: `w-full max-w-md mx-auto rounded-full h-4 mt-2 shadow-inner ${T.progressTrack}`, children: _jsx("div", { className: `${T.progressBar} h-4 rounded-full transition-all duration-500`, style: { width: `${porcentaje}%` } }) }), _jsx("button", { className: `mt-4 font-semibold px-4 py-2 rounded shadow transition-colors ${T.resetBtn}`, onClick: () => setCursosAprobados([]), children: "Reiniciar malla" })] }), _jsxs("div", { className: "mx-auto w-full max-w-[1600px]", children: [_jsx("div", { className: "grid grid-cols-10 gap-4 mb-2", children: anos.map(({ año, span }, index) => (_jsx("div", { style: { gridColumn: `span ${span} / span ${span}` }, className: `text-center font-bold py-2 rounded-md text-sm ${T.years}`, children: año }, index))) }), _jsx("div", { className: "grid grid-cols-10 gap-4 mb-6", children: romanos.map((r, i) => (_jsx("div", { className: `text-center font-bold py-1 rounded-md shadow text-sm ${T.sems}`, children: r }, i))) }), _jsx("div", { className: "grid grid-cols-10 gap-x-6 gap-y-8", children: Array.from({ length: 10 }).map((_, semestreIndex) => {
                            const semestre = semestreIndex + 1;
                            const cursos = cursosPorSemestre[semestre] || [];
                            return (_jsx("div", { className: "flex flex-col gap-6 items-center", children: cursos.map((curso) => {
                                    ...
                                    const btnClass = aprobado
                                        ...
                                        : desbloqueado
                                            ? T.cardUnlocked
                                            : T.cardBlocked;
                                    return (_jsxs("div", { className: "relative group w-fit", children: [_jsxs("button", { className: `w-[8rem] h-[5.5rem] flex flex-col justify-center items-center rounded-md shadow text-center text-[10px] font-medium transition-all duration-300 ease-in-out transform ${btnClass} ${aprobado || desbloqueado ? "hover:scale-105" : ""}`, disabled: !desbloqueado && !aprobado, onClick: () => toggleCurso(curso.id), children: [_jsx("div", { className: `absolute top-1 left-2 text-[10px] font-bold opacity-80 ${T.codeTint}`, children: curso.id }), _jsx("div", { className: "font-semibold text-[13px] leading-snug text-center", children: curso.nombre }), _jsxs("div", { className: `text-[11px] mt-1 ${T.creditsTint}`, children: ["Cr\u00E9ditos: ", curso.creditos] }), aprobado && _jsx("span", { className: "absolute top-1 right-1 text-white text-lg", children: "\u2728" })] }), !desbloqueado && !aprobado && curso.requisitos.length > 0 && (_jsxs("div", { className: `absolute -top-20 left-1/2 -translate-x-1/2 w-64 rounded-md shadow-lg px-3 py-2 z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none ${modoOscuro
                                                    ? "bg-blue-900 border border-blue-700 text-blue-100"
                                                    : "bg-white border border-teal-300 text-teal-900"}`, children: ["Debes aprobar:", _jsx("ul", { className: "list-disc list-inside mt-1", children: curso.requisitos.map((reqId) => {
