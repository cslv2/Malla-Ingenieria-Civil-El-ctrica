...
const LIGHT_REDORANGE = {
  ...
  cardUnlocked: "bg-[#ffe4ea] text-[#5c0010] motion-safe:hover:bg-[#f5abb0]",
...
const DARK = {
  ...
  cardUnlocked: "bg-purple-700/80 text-white motion-safe:hover:bg-purple-600",
...
const App: React.FC = () => {
  ...
  // ✅ usa la extensión correcta del archivo en /public
  const logoSrc = import.meta.env.BASE_URL + "logo.jpeg";

  return (
    <div
      className={`relative min-h-screen p-3 sm:p-6 pb-24 sm:pb-20 overflow-x-hidden transition-colors duration-500 ${T.appBg}`}
    >
      {/* Tema */}
      <button
      ...
      {/* ESCRITORIO */}
      <div className="hidden sm:block mx-auto w-full max-w-[1200px] lg:max-w-[1600px] overflow-x-auto">
        ...
        {Array.from({ length: 10 }).map((_, idx) => {
          ...
          return (
            <section key={semestre} className="mb-8">
              ...
              <div className="flex gap-4 pb-2 overflow-x-auto scroll-row">
                {cursos.map((curso) => (
                  <CourseCard
                    key={curso.id}
                    curso={curso}
      ...
      {/* Footer fijo */}
      <footer
        className={`fixed bottom-0 left-0 right-0 h-16 z-50 shadow-md
          ${modoOscuro ? "bg-blue-950 text-blue-200" : "bg-slate-100 text-slate-700"}`}
      >
        <div className="h-full max-w-[1600px] mx-auto px-4 flex items-center justify-center gap-3">
          <img src={logoSrc} alt="Logo" className="h-8 w-auto" />
          <p className="text-sm text-center">
            Desarrollado por <span className="font-semibold">Camila Silva</span> © {new Date().getFullYear()}
          </p>
        </div>
      </footer>
    </div>
